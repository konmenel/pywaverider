...
def config_create_window() -> None:
    ...
    # Click Functions
    def click_start():
        """The function that runs after you press the start button. Creates the configuration."""
...
def modify_config() -> None:
    """The function that modifies the configuration file from a config.CONFIG dictionary."""
    # Modify config.py
...
def results_load_pickle(second_output: bool=False) -> Union[dict, Tuple[dict, str]]:
    """The function that loads a saved pickle result."""
    window = tk.Tk()
    window.withdraw()
...
def results_load_json(second_output: bool=False) -> Union[dict, Tuple[dict, str]]:
    """The function that loads a saved json result."""
    window = tk.Tk()
    window.withdraw()
    window.filename = filedialog.askopenfilename(title="Select Results",
                    initialdir=((pathlib.Path(__file__)).parents[1]).joinpath('Results'),
